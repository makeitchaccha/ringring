...
import (
	...
	"github.com/makeitchaccha/ringring/ringring/commands"
	"github.com/makeitchaccha/ringring/ringring/components"
	"github.com/makeitchaccha/ringring/ringring/handlers"
)
...
func main() {
	...
	cfg, err := ringring.LoadConfig(*path)
	if err != nil {
	...
	setupLogger(cfg.Log)
	slog.Info("Starting Ringring...", slog.String("version", Version), slog.String("commit", Commit))
	slog.Info("Syncing commands", slog.Bool("sync", *shouldSyncCommands))

	b := ringring.New(*cfg, Version, Commit)
...
func setupLogger(cfg ringring.LogConfig) {
